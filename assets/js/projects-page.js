document.addEventListener('DOMContentLoaded', () => {
  const container = document.getElementById('project-list-all');
  ...
  container.innerHTML = projects.map((project, i) => {
    ...
    return `
      <a href="${detailUrl}" class="project-card fade-in-up fade-delay-${(i % 3) + 1}">
  ...
  if (window.detectPortraitImages) window.detectPortraitImages('#project-list-all');
});
