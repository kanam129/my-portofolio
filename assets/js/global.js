document.addEventListener('DOMContentLoaded', () => {
    ...
    if (footerName) footerName.textContent = `${new Date().getFullYear()} ${config.name}. All rights reserved.`;
    ...
    const socialLinksHTML = Object.entries(config.socials).map(([key, value]) => {
        if (value && socialSvgs[key]) {
            return `<a href="${value}" target="_blank" rel="noopener noreferrer" class="text-slate-500 hover:text-slate-800 transition">${socialSvgs[key]}</a>`;
        }
        return '';
    }).join('');

    // Populate Social Media Links in Hero
    const socialLinksHero = document.getElementById('social-links-hero');
    if(socialLinksHero) socialLinksHero.innerHTML = socialLinksHTML;

    // Populate Social Media Links in Footer
    const socialLinksFooter = document.getElementById('social-links-footer');
    if(socialLinksFooter) socialLinksFooter.innerHTML = socialLinksHTML;
});
